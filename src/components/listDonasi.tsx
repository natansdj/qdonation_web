...
const ListDonasi = memo(
  ({ data, loading }: { data: ItemProgram[]; loading: boolean }) => {
    return (
      <div className="flex flex-col px-[15px]">
        {loading ? (
          [1, 2, 3, 4, 5].map((item) => (
            <div
              key={item}
              className="p-[10px] border-[#EDEDED] border rounded-[10px] animate-pulse mt-[10px]"
            >
          ...
          data.map((item, index) => {
            ...
            return (
              ...
              >
                <CardImage
                  url={item.program_images?.[0]}
                  category={item?.program_categories?.[0]}
                />
                <div className="text-[16px] font-medium text-[#000] my-[10px]">
                  {item.name}
                </div>
                <Progress percent={item.current_progress} />
                <div className="flex justify-between mb-[5px] mt-[15px]">
                  {item.view_type === "percentage" ? (
                    <div className="text-[16px] font-medium text-[#000]">
                     {item.current_progress}%
                    </div>
                  ) : (
                    <div className="text-[16px] font-medium text-[#000]">
                      Rp{parsingCurrencyRupiah(`${item.has_target}`)}
                    </div>
                  )}

                  <div className="text-[14px] font-medium text-[#111111]">
                    {moment(item.period_end_date).diff(moment(), "days")}{" "}
                    <span className="text-[#787878]">Hari lagi</span>
                  </div>
                </div>
              </Link>
            );
