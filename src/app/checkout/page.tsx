...
export default function Detail() {
  ...
  const paymentLoading = useAppSelector(
    (state) => state.payment.paymentLoading
  );
  ...
  const paymentResponse = useAppSelector(
    (state) => state.payment.paymentResponse
  );
  ...
  useEffect(() => {
    ...
    timeout = setTimeout(() => {
      if (
        paymentResponse?.donation_id &&
        paymentResponse?.payment_info?.donation_payment_id
      ) {
        if (token)
          router.push(`/thanks?donation_id=${paymentResponse?.donation_id}`);
        else
          router.push(
            `/status?donation_id=${paymentResponse?.donation_id}&payment_id=${paymentResponse?.payment_info?.donation_payment_id}`
          );
      }
    }, 1000);
  }, [paymentResponse]);
  ...
  const payment = () => {
    if (token && value) {
      ...
      if (choose.type_code == "card") {
        if (
          choose.card_cvn &&
          choose.card_account_number &&
          choose.card_exp_year &&
          choose.card_exp_month
        ) {
      ...
      <div className="">
        ...
        <div className="flex flex-col gap-[10px]">
          <div className="p-[15px] bg-[#fff]">
            <div className="text-[18px] font-medium text-[#1A1B1E] mb-[15px]">
              Rincian Donasi
            </div>
        ...
        <div className="footer-button">
          <div className="p-[15px] bg-white flex justify-between items-center border-[#E5E6EB] border-t">
            <div>
              <div className="text-[12px] text-[#1A1B1E] font-medium mb-[5px]">
                Total Bayar
              </div>
              <div className="text-[20px] text-[#1A1B1E] font-medium">
                Rp{parsingCurrencyRupiah(`${value || 0}`)}
              </div>
            </div>
            <div onClick={payment} className="min-w-[180px]">
              <Button title="Bayar Sekarang" type="warning" />
            </div>
          </div>
        </div>
      </div>
